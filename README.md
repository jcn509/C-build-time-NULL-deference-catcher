...
{
    {
        void null_dereference_of_my_nc_ptr_possible_in_file_compilation_failure_dereference_read_null_c_on_line_13 ();
        if (_PRIVATE_NC_PTR_DO_NOT_TOUCH_my_nc_ptr == NULL) {
            null_dereference_of_my_nc_ptr_possible_in_file_compilation_failure_dereference_read_null_c_on_line_13 ();
        } 
    };
...
(null_dereference_of_my_nc_ptr_possible_in_file_compilation_failure_dereference_read_null_c_on_line_13 in the example above.)
...
```c
#define NC_PTR_SAFE_FILE_NAME compilation_fixed_dereference_read_null_c
...
```c
#define NC_PTR_SAFE_FILE_NAME no_compilation_problems
...
have whatever optimisations relate to dead code optimisation enabled. With Clang and GCC if you are
doing a debug build make sure you have -Og enabled. (Note: CMake will not add and optimisation
...
the examples that do not compile due to NULL references and make sure that the correct
